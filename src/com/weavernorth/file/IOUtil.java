...
public class IOUtil {
   ...
   */
    ...
    */ 
    ...
    public static boolean deleteDirectory(String dir) {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            // 删除子目录
                ...
                flag = IOUtil.deleteDirectory(files[i]
                        .getAbsolutePath());
                if (!flag)
                    break;
            }
    ...
    public static void judeDirExistsAll(File file) {
        if (file.exists()) {
            if (file.isDirectory()) {
                ...
                System.out.println("the same name file exists, can not create dirs");
            }
        } else {
            System.out.println("dirs not exists, create it ...");
            file.mkdirs();
        }
    ...
    /**
     * 以行为单位写出文件，常用于写面向行的格式化文件
     * @param file  写出文件全名
     * @param content   写入内容，在换行的索引处请增加换行符
     */
    public static void writeFileByLines(String file,String content){
